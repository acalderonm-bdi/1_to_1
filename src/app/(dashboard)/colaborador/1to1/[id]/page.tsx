...
export default async function OneOnOneDetailPage({ params }: PageProps) {
  ...
  // Obtener la 1:1 con participantes
  ...
  const isParticipant =
    leader?.id === user.id || collaborator?.id === user.id
  ...
  // Obtener minuta del usuario
  const { data: myMinute } = await supabase
    .from('minutes')
    .select('raw_content')
    .eq('one_on_one_id', params.id)
  ...
  // VoBo del usuario actual
  const { data: myVobo } = await supabase
    .from('vobos')
    .select('confirmed')
    .eq('one_on_one_id', params.id)
    .eq('user_id', user.id)
  ...
  // Acuerdos pendientes de 1:1s ANTERIORES (para followup modal)
  const { data: prevAgreements } = await supabase
    .from('agreements')
    .select(
      'id, description, due_date, one_on_ones!inner(leader_id, collaborator_id, scheduled_at)',
    )
  ...
  const pendingPrevAgreements = (prevAgreements ?? []).map((a) => ({
    id: a.id,
  ...
  const voboValue = (myVobo as { confirmed: boolean } | null)?.confirmed ?? null
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {/* Info de participantes y modalidad */}
      <Card>
        <CardContent className="pt-4 pb-4 space-y-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span>
              ...
              <span className="text-slate-400">(líder)</span> con{' '}
      ...
      {/* Minuta + Acuerdos + VoBo (solo si pasó la reunión) */}
